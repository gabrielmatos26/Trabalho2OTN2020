...
class CheckersGameRandom:
    ...
    def startGame(self, num_matches, player, num_executions=1):
        self.game = Game()
        self.sr_branca = 0
        self.draw_branca = 0
        self.sr_preta = 0
        ...
        for e in tqdm(range(num_executions)):
            if player == 1:
                print("Avaliando jogos dos pesos para peças brancas contra peças pretas aleatório")
                ...
                print("Avaliando jogos dos pesos para peças pretas contra peças brancas aleatório")
                for i in tqdm(range(num_matches), leave=False):
                    while not self.game.is_over():
                        if self.game.whose_turn() == 2:
